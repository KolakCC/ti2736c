...
public class Perceptron {
    ...
    public void train(FeatureVector fv) {
        ...
        // add code here
        System.out.printf("check: %d == %d %n", fv.getLabel(), predict(fv));
        if (fv.getLabel() == predict(fv)) return;
        for (int i = 0; i < weights.size(); i++) {
            //weights.set(i, weights.get(i) + learningRate * (fv.getLabel() - predict(fv)) * fv.get(i));
            weights.set(i, weights.get(i) + learningRate * fv.getLabel() *  fv.get(i));
            System.out.println(learningRate * fv.getLabel() *  fv.get(i));
        }
    }
